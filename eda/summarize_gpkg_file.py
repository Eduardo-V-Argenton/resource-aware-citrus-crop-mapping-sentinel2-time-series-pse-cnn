...
for file in arquivos_gpkg:
    ...
    if COLUNA_NM not in gdf.columns:
        print(f"[{file}] ATENÇÃO: Coluna COLUNA_NM não encontrada. Pulando...")
        continue

    # Agrupa por REGIÃO e por CLASSE simultaneamente
    # Assim garantimos que as cidades não se misturem
    df_agrupado = gdf.groupby([COLUNA_NM, 'mapbiomas_class'])['area_ha'].sum().reset_index()
    
    # Pega a lista de regiões únicas que existem dentro deste arquivo específico
    regioes_no_arquivo = df_agrupado[COLUNA_NM].unique()
    
    for regiao in regioes_no_arquivo:
        # Isola os dados apenas desta região
        df_regiao = df_agrupado[df_agrupado[COLUNA_NM] == regiao]
