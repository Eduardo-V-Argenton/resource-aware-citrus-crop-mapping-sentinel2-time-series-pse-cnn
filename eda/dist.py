...
PASTA_TENSORES = '/mnt/SSD_SATA/Tensores_Treino/'
...
for l, linha in enumerate(df_csv.itertuples(index=False)):
    ...
    try:
        ...
        b2 = t_10[:, 0, :, :]   # BLUE
        b3 = t_10[:, 1, :, :]   # GREEN (Restaurado para a máscara e TCARI)
        b4 = t_10[:, 2, :, :]   # RED
        b8 = t_10[:, 3, :, :]   # NIR
        ...
        t_10[mascara_destruicao] = np.nan
        
        is_all_null = True
        null_count = 0
        
        for i in range(t_10.shape[0]):
            if np.isnan(t_10[i]).all():
